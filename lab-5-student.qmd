...
teacher_evals <- read_csv("data/data-raw/teacher_evals.csv")
...
> There are 22 columns and 8,015 rows in the data set. The mean value for the response variable (SET_score_avg) is 4.392, and the range is 1-5. Given the high average score and the fact that the first quartile is 4, low scores must not be common. There is a combination of categorical and numerical variables. Some columns, like time_of_day or seniority, could be redefined depending on how we want to treat them in the analysis. For example, seniority may be ordinal and better categorized as a factor. Certain columns, like SET_score_1sem and class_duration, have a disproportionately high number of NAs.
...
> I think- if the units of observations are the teachers- that they are identified by the teacher_id column.
...
teacher_evals |> 
  ...
  select(course_id, 
         ...
         seniority, 
         sex) |> 
write_csv("data/data-clean/teacher_evals_clean.csv")

teacher_evals_clean <- read_csv("data/data-clean/teacher_evals_clean.csv")
teacher_evals_clean <- janitor::clean_names(teacher_evals_clean) 
...
# code chunk for Q5
unique(teacher_evals_clean[,1])
unique(teacher_evals_clean[,2])

```

> There are 921 unique Course IDs and 294 unique Teacher IDs.
...
# code chunk for Q6
teacher_evals_clean |> 
  filter(is.na(question_no) | is.na(no_participants) | is.na(resp_share) | is.na(set_score_avg) | is.na(percent_failed_cur)) |> 
  group_by(teacher_id, course_id)
  
```

> Course ID PAB3SE004PA with Teacher ID 56347 has missing values in percent_failed_cur.
...
mean(teacher_evals_clean$seniority)

table(teacher_evals_clean$sex)

```

> 4,270 have doctorate degrees, 1,808 have master's degrees, 118 had professional degrees, and 467 have no degrees. There are 11 seniority levels; the average level is 5.5 and the median is 6. The gender balance is fairly equal, with 3,199 females and 3,464 males.
...
> There are 48 teacher-course ID combinations that asked all 9 questions.
...
# code chunk for Q9
#view highest and lowest scores overall
teacher_evals_clean |>
  select(set_score_avg) |> 
...
#answer the question: highest and lowest rated teachers
teacher_evals_clean |>
  group_by(teacher_id) |>
  summarize(total_score_avg = mean(set_score_avg)) |> 
  filter(total_score_avg %in% c(1,5)) |> 
  arrange(total_score_avg)
```
...
teacher_evals_clean |> 
  ...
  group_by(teacher_id) |> 
  summarize(avg_resp_rate = mean(resp_share, na.rm = TRUE)) |> 
  arrange(desc(avg_resp_rate))
  #arrange(avg_resp_rate)

```

> Teacher 59338 had the lowest average response rate across all classes at 1.48%. Teacher 101508 had the highest rate at 52.17%.
