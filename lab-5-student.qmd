...
> There are 22 columns and 8,015 rows in the data set. The mean value for the response variable (SET_score_avg) is 4.392, and the range is 1-5. Given the high average score and the fact that the first quartile is 4, low scores must not be common.
>There is a combination of categorical and numerical variables. Some columns, like time_of_day or seniority, could be redfined depending on how we want to treat them in the analysis. For example, seniority may be ordinal and better categorized as a factor. Certain columns, like SET_score_1sem and class_duration, have a disproportionately high number of NAs. 
